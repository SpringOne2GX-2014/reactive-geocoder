 ...
 * reactor.rx.Stream}j to be processed.
 * <p>
...
public class GeoNearPredicate implements Predicate<Location> {
	...
	public boolean test(Location compareTo) {
		if (home.getX() == compareTo.getCoordinates().getX()
				&& home.getY() == compareTo.getCoordinates().getY()) {
			return true;
		}

		double lat1 = home.getX();
		double lat2 = compareTo.getCoordinates().getX();
		double lon1 = home.getY();
		double lon2 = compareTo.getCoordinates().getY();
